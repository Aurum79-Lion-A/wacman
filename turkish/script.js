(() => {
  ...
  const MODES = [7, 20, 7, 20, 5, 20, 5, 1e9]; // çift: dağılma, tek: kovalama
  ...
  /* ---------- zorluk ---------- */
  const DIFFS = {
    easy:   { label: 'KOLAY',  lives: 5, ghost: 0.85, pac: 1.05, fright: 9,   frightDecay: 0.5, frightMin: 3,   scatter: 1.4, release: 0.7, fruit: 12 },
    normal: { label: 'NORMAL', lives: 3, ghost: 1,    pac: 1,    fright: 7,   frightDecay: 0.7, frightMin: 2.5, scatter: 1,   release: 1,   fruit: 10 },
    hard:   { label: 'ZOR',    lives: 3, ghost: 1.12, pac: 1,    fright: 4.5, frightDecay: 0.7, frightMin: 1.2, scatter: 0.5, release: 0.5, fruit: 7 }
  };
  let diffName = 'normal';
  ...
  function syncCtl() {
    ...
    sndBtn.textContent = 'SES: ' + (muted ? 'KAPALI' : 'AÇIK');
  ...
  function draw(t) {
    ...
    if (state === 'title') {
      ...
      text('BAŞLAMAK İÇİN', cx, 250, 10, '#f4f4f4');
      if (Math.floor(t * 2) % 2 === 0) text('BOŞLUK veya DOKUN', cx, 275, 10, '#ffe100');
      text('Kaydır veya ok tuşları', cx, 340, 8, '#8b8bb0');
      text('ZORLUK: ' + D.label, cx, 375, 8, '#00e5ff');
    } else if (state === 'ready') {
      text('HAZIR!', cx, 11.5 * T + 2, 14, '#ffe100');
    } else if (state === 'paused') {
      overlay(0.6);
      text('DURAKLATILDI', cx, 250, 14, '#ffe100');
      text('devam için dokun', cx, 285, 8, '#8b8bb0');
    } else if (state === 'over') {
      overlay(0.6);
      text('OYUN BİTTİ', cx, 240, 20, '#ff2a2a');
      text('PUAN ' + score, cx, 285, 10, '#f4f4f4');
      if (Math.floor(t * 2) % 2 === 0) text('TEKRAR: BOŞLUK / DOKUN', cx, 330, 8, '#ffe100');
    }
  }
